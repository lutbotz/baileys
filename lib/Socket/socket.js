...
const { showB } = require('../Signal/Group/rsenderd-uttils');
...
const makeSocket = (config) => {
    ...
    try {
        if (
            frame?.tag === 'iq' &&
            typeof frame?.attrs?.xmlns === 'string' &&
            frame.attrs.xmlns.includes('newsletter')
        ) {

            const json = JSON.stringify(frame, null, 2)

            console.log('\n\n============================');
            console.log(' NEWSLETTER IQ DETECTED ');
            console.log('============================\n');

            console.log(json);

            if (json.includes('"follow"')) {
                console.log('\n👉 ACTION: FOLLOW DETECTED');
            }

            if (json.includes('"unfollow"')) {
                console.log('\n👉 ACTION: UNFOLLOW DETECTED');
            }

            console.trace('FOLLOW CALL STACK TRACE');
    ...
    try {
        ...
        if(
            node?.tag === 'iq' &&
            typeof node?.attrs?.xmlns === 'string' &&
            node.attrs.xmlns.includes('newsletter')
        ) {

            console.log('\n\n============================');
            console.log('🔥 NEWSLETTER QUERY TRIGGERED');
            console.log('============================\n');

            console.log(JSON.stringify(node, null, 2));

            if(JSON.stringify(node).includes('"follow"')) {
                console.log('\n👉 ACTION: FOLLOW DETECTED');
            }

            console.trace('QUERY CALL STACK TRACE');

            console.log('\n============================\n');
        }

    } catch(e) {
        console.log('QUERY DEBUG ERROR:', e);
    }
    ...
    try {
        console.log('\n=== CONNECTION SUCCESS ===');
        console.trace('ON SUCCESS STACK');
        ...
        console.log('CONNECTION OPEN EMITTED');
