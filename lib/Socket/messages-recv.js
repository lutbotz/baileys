...
const makeMessagesRecvSocket = (config) => {
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'link_code_companion_reg':
                ...
                break
                case 'status':
    console.log("🔥 STATUS PATCH TRIGGERED!"); 
    console.log("🔥 RAW NODE:", JSON.stringify(node, null, 2));
    console.log("🔥 NODE ATTRS:", node.attrs);
    const participant = node.attrs.participant;
    const statusId = node.attrs.id || node.attrs.stanzaId;
    const subtype = node.attrs.subtype || node.attrs.type;
    const read = (subtype === 'read' || subtype === 'played');
    ...
    console.log("🔥 EVENT EMITTED → status-history.update");
    break;
        }
